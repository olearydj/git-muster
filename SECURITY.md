...
The archives attached to a GitHub release are the files PyPI holds. A publishing run attaches the artifacts it just published; a recovery run, which publishes nothing, downloads the archives from PyPI and verifies their SHA-256 digests before attaching them. The 0.3.1 archives were attached by hand before that automation existed, and were replaced through the recovery path on 2026-08-30, so its wheel no longer differs from the published one. Treat PyPI, whose files carry attestations, as the authority for any release.
